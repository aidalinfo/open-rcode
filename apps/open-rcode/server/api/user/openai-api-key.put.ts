...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Encrypt and save
    user.openaiApiKey = encrypt(body.openaiApiKey)
    await user.save()

    return { success: true, message: 'OpenAI API key saved successfully' }
  } catch (error: any) {
    if (error.statusCode) throw error
    throw createError({ statusCode: 500, statusMessage: `Failed to save OpenAI API key: ${error.message}` })
  }
})
