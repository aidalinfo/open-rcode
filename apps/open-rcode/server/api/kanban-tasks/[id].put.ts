...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { title, message, error, status, plannifiedAt, taskId: linkedTaskId } = body
    ...
    if (plannifiedAt !== undefined) task.plannifiedAt = plannifiedAt
    if (linkedTaskId !== undefined) task.taskId = linkedTaskId
    ...
    throw createError({
      statusCode: 500,
      statusMessage: `Failed to update kanban task: ${error.message}`
    })
  }
})
